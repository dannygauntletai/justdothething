...
This document defines the requirements for a productivity app designed to assist users in maintaining focus and managing tasks effectively. The app integrates artificial intelligence (AI) to monitor user activity, deliver real-time interventions, and provide personalized productivity insights. It will be accessible via a **web dashboard** for detailed control and analytics.
...
JustDoTheThing will be available through a web dashboard for detailed views and quick interactions.
...
- **Features**:
  ...
  - **AI Analysis**: Use a pre-trained image classification model (MobileNet via TensorFlow.js) **locally in the browser** to classify screenshots as "work" or "non-work" based on open applications or content. If the screenshot is non-work, send the first one to GPT-4 with Vision for enhanced contextual analysis.
  ...
  - **Style Customization**: Enable users to choose yelling styles (e.g., "Cheerful Coach," "Drill Sergeant") via the web dashboard, adjustable during use.
...
- **Features**:
  - **Speech Input**: Activate speech-to-text with a button in the web dashboard.
...
- **Features**:
  ...
  - **Chronotype Analysis**: Use TensorFlow.js with a time-series model (e.g., LSTM) **locally in the browser** to identify peak productivity times (e.g., morning person, night owl).
...
- **Frontend**: React with Vite for the web dashboard.
- **Backend**: Node.js for API management and data processing.
- **Database**: Supabase for secure storage of tasks, settings, and data.
- **AI/ML Tools**:
  - **Screenshot Analysis**: TensorFlow.js with MobileNet for work/non-work classification (local).
...
- **Web Dashboard**: Captures screenshots, tracks activity, and records speech; sends data to the backend where necessary.
- **Backend**: Handles API requests, integrates external AI services, and manages Supabase data.
- **Local Processing**: Handles AI tasks like screenshot classification and chronotype analysis using TensorFlow.js in the browser.

### **6.3 APIs and Integrations**

- **Yell Mode**: TensorFlow.js (local screenshot analysis), ElevenLabs (text-to-speech).
- **Yap Mode**: OpenAI Whisper (speech-to-text), GPT-4 (NLP), Twitter API (tool scanning).
- **Flow Mode**: TensorFlow.js for productivity analysis (local).
...
1. **Onboarding**:
   - Grant permissions (screen, microphone) during web app setup.
   - Set preferences (e.g., yelling style).

2. **Mode Activation**:
   - Toggle Yell, Yap, or Flow Modes via the web dashboard; modes can run simultaneously.
...
5. **Flow Mode**:
   - Collects data during use and offers real-time tips.
   - Provides detailed insights on the dashboard.

6. **Dashboard Access**:
   - Access the web dashboard to manage tasks, settings, and insights.
...
- **Implementation Steps**:
  1. Set up screenshot capture and inactivity detection in the web app.
  2. Implement AI for work/non-work classification using local TensorFlow.js.
  3. Integrate text-to-speech for yells.
  4. Add customization options via the web dashboard.
